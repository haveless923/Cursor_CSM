...
-- ============================================
-- 4. 行业新闻表 (industry_news) - 如果使用行业情报功能
-- ============================================
...
-- ============================================
-- 5. 新闻收藏表 (news_favorites) - 如果使用行业情报功能
-- ============================================
...
-- ============================================
-- 8. Row Level Security (RLS) 策略（可选但推荐）
-- ============================================
-- 启用 RLS
ALTER TABLE customers ENABLE ROW LEVEL SECURITY;
ALTER TABLE next_step_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE news_favorites ENABLE ROW LEVEL SECURITY;

-- customers 表策略：用户可以查看自己负责的客户，admin可以查看所有
...
-- customers 表策略：用户可以创建客户
CREATE POLICY "Users can create customers" ON customers
  FOR INSERT
  WITH CHECK (true);

-- customers 表策略：用户可以更新自己负责的客户，admin可以更新所有
...
-- customers 表策略：用户可以删除自己负责的客户，admin可以删除所有
...
-- next_step_history 表策略：用户可以查看自己创建的记录
CREATE POLICY "Users can view own history" ON next_step_history
  FOR SELECT
  USING (
...
-- next_step_history 表策略：用户可以创建历史记录
CREATE POLICY "Users can create history" ON next_step_history
  FOR INSERT
  WITH CHECK (true);

-- news_favorites 表策略：用户只能查看自己的收藏
CREATE POLICY "Users can view own favorites" ON news_favorites
  FOR SELECT
  USING (auth.uid()::text = user_id::text);

-- news_favorites 表策略：用户只能管理自己的收藏
CREATE POLICY "Users can manage own favorites" ON news_favorites
  FOR ALL
  USING (auth.uid()::text = user_id::text)
